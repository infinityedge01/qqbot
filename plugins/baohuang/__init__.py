...
async def rangwei(session):
    ...
    if session.current_arg == '' and session.event.group_id in is_baohuang_open and table != None:
        ...
        await bot.send_private_msg(user_id = table.huangdi_id, message = message.MessageSegment.text(tile_dict_to_string(table.players[table.huangdi_id].tiles)))
    ...
    if table.mutiple == 4: 
        if table.players[table.huangdi_id].get_order() == 1:
            for player in table.player_id:
                ...
                else: table.huangdi_id: table.players[player].point = -basepoint * table.mutiple
        else:
            for player in table.player_id:
                if player == table.huangdi_id: table.players[player].point = -basepoint * table.mutiple * 4
                else: table.huangdi_id: table.players[player].point = basepoint * table.mutiple
        ...
        for player in table.player_id:
            ...
            if player == table.huangdi_id: table.players[player].point = basepoint * table.mutiple * K
            else: table.huangdi_id: table.players[player].point = -basepoint * table.mutiple * K
    for i in range(5): 
